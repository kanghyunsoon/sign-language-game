...
9. 모델은 프레임을 독립적으로 분류하고, AI 서버가 프레임 결과를 시간 기준으로 집계한다.
...
프레임 모델 `v1`은 31개 지문자와 손이 검출된 비지문자 자세인 `none`을 포함한 32개 클래스를 출력한다. `transition`은 모델 클래스가 아니라 AI 서버가 연속 프레임의 변화로 판정하는 상태다. 추론 중 프론트엔드는 `none`이나 `transition`을 결정해서 보내지 않는다.
...
브라우저 성능 저하로 프레임을 건너뛸 수 있지만, 이미 생성한 프레임의 타임스탬프를 변경하거나 중복 전송하면 안 된다. AI 서버는 촬영 타임스탬프 기준으로 순서를 검증하고 실제 수신 간격을 유지한다.
...
추론 모드에서는 판정을 시작하기 전에 `target.set`을 전송한다. 목표가 변경되면 같은 세션에서 다시 전송할 수 있으며 AI 서버는 기존 후보와 집계 구간을 초기화한다.
...
`targetLabelId`는 `labels.json`의 지문자 31개 ID 중 하나여야 한다. `none`, `transition`, 숫자 ID는 허용하지 않는다. `setTimestampMs`는 세션 시작 기준 단조 증가 밀리초이며, 해당 시각 이후의 프레임만 새 목표 판정에 사용한다.
...
- 서버는 누락 프레임을 합성하지 않고 실제 타임스탬프와 유효 프레임 비율로 집계한다.
...
| 항목 | 기준값 | 의미 |
| --- | ---: | --- |
| 영상 수집 최소 유지시간 | 1,200ms | 안정 프레임을 추출할 영상 구간의 최소 길이 |
...
사진 학습 데이터에는 유지시간 조건을 적용하지 않는다. 영상에서 학습 프레임을 추출할 때와 서비스 결과를 확정할 때는 실제 촬영 타임스탬프 기준 1,200ms를 사용한다. 모든 기준값의 기계 판독 원본은 `ai/config/recognition-policy.json`이다.
...
전송 규격과 실제 모델 입력은 구분한다. 서버는 원본 랜드마크를 보존하고 검출된 프레임마다 다음 순서로 모델 입력을 만든다.

1. `worldLandmarks`가 21개의 유한한 3차원 좌표인지 검증한다.
2. 손목 인덱스 `0`을 원점으로 이동한다.
...
모델은 각 프레임을 독립적으로 분류하고 지문자 31개와 `none`의 확률을 반환한다. 손 미검출 프레임은 모델에 넣지 않고 AI 서버가 `none` 상태로 처리한다. 프레임 간 좌표 차이와 12프레임 윈도우는 모델 입력에 포함하지 않는다.
...
AI 서버는 `target.set`으로 받은 목표 클래스의 프레임별 확률을 촬영 타임스탬프 기준으로 집계한다.

1. 목표 10 FPS로 수신하되 프레임 개수만으로 유지시간을 계산하지 않는다.
2. 목표 설정 이후 1,200ms 구간에서 유효 프레임 비율이 80% 이상인지 확인한다.
3. 유효 프레임의 목표 클래스 평균 신뢰도가 기본 임계값 0.80 이상인지 확인한다.
4. 200ms 이하의 미검출 또는 낮은 신뢰도는 일시적 실패로 허용한다.
5. 목표 변경, 200ms 초과 실패 또는 300ms 이상 해제 상태가 발생하면 집계를 초기화한다.
6. 모든 조건을 충족하면 `confirmed`를 한 번 반환한다.

신뢰도 임계값은 검증 데이터로 보정해 클래스별 값으로 확장할 수 있다. 기본값과 시간 기준은 `recognition-policy.json`에서만 관리한다.

## 11. 학습 구간 생성 기준

| 구간 | 생성 기준 |
| --- | --- |
| 사진 지문자 클래스 | 유지시간 없이 품질 검사를 통과한 단일 프레임 |
| 영상 지문자 클래스 | 목표 자세가 안정된 1.2초 이상의 구간에서 추출한 프레임 |
| `transition` | 모델 학습에서 제외하고 서버 상태 검증에 사용 |
| `none` | 손은 검출되지만 정의된 지문자 자세가 아닌 프레임 |
| 손 미검출 | 별도 클래스 학습보다 손 검출 여부 규칙에 우선 사용 |

유효 영상 구간은 전체 샘플의 90% 이상에서 손이 검출되어야 한다. 시작과 종료 시점의 자세 생성 및 해제 프레임은 지문자 정답에서 제외한다. 참가자 정보가 없는 외부 데이터는 최종 검증 및 테스트가 아닌 학습 데이터로만 사용한다.
...
- [ ] 손 개수를 1개로 제한한다.
- [ ] 목표 10 FPS로 샘플링한다.
- [ ] 추론 시작 및 목표 변경 시 `target.set`을 전송한다.
...
- [ ] 세션과 프레임 스키마를 검증한다.
- [ ] 타임스탬프 기준으로 프레임 순서와 유지시간을 검증한다.
- [ ] 왼손과 오른손을 동일 기준으로 정규화한다.
- [ ] 단일 프레임 모델 입력 `[batchSize, 63]`을 생성한다.
- [ ] `target.set` 수신 시 기존 후보와 집계 구간을 초기화한다.
- [ ] 실제 타임스탬프 기준 1,200ms 동안 프레임 결과를 집계한다.
