...
describe("DefaultContinuousSignDecoder", () => {
  ...
  it("prevents repeats while the same pose is held", () => {
    const listener = vi.fn(); decoder.subscribe(listener);
    confirm(decoder, "ㄱ", 1);
    decoder.pushFrame(predictionOnly(120, "ㄱ", 4)); decoder.pushFrame(predictionOnly(160, "ㄱ", 5)); decoder.pushFrame(predictionOnly(200, "ㄱ", 6));
    expect(listener.mock.calls.filter(([event]) => event.type === "SIGN_CONFIRMED")).toHaveLength(1);
  });

  it("releases after a changed pose is held, then confirms the same symbol again", () => {
    const listener = vi.fn(); decoder.subscribe(listener); confirm(decoder, "ㄱ", 1);
    decoder.pushFrame(frame(130, changed())); decoder.pushFrame(frame(190, changed()));
    expect(listener).toHaveBeenCalledWith(expect.objectContaining({ type: "HAND_RELEASED", previousSymbol: "ㄱ" }));
    decoder.pushFrame(frame(240, base())); decoder.pushFrame(frame(290)); decoder.pushFrame(frame(340)); decoder.pushFrame(frame(380));
    decoder.pushFrame(predictionOnly(380, "ㄱ", 4)); decoder.pushFrame(predictionOnly(390, "ㄱ", 5)); decoder.pushFrame(predictionOnly(400, "ㄱ", 6));
    expect(listener.mock.calls.filter(([event]) => event.type === "SIGN_CONFIRMED" && event.symbol === "ㄱ")).toHaveLength(2);
  });

  it("releases to a stable different symbol without removing the hand", () => {
    const listener = vi.fn(); decoder.subscribe(listener); confirm(decoder, "ㄱ", 1);
    decoder.pushFrame(predictionOnly(130, "ㄴ", 4)); decoder.pushFrame(predictionOnly(150, "ㄴ", 5));
    expect(listener).toHaveBeenCalledWith(expect.objectContaining({ type: "HAND_RELEASED", previousSymbol: "ㄱ" }));
    decoder.pushFrame(predictionOnly(170, "ㄴ", 6)); decoder.pushFrame(predictionOnly(190, "ㄴ", 7));
    expect(listener).toHaveBeenCalledWith(expect.objectContaining({ type: "SIGN_CONFIRMED", symbol: "ㄴ" }));
  });

  it("releases after a short no-hand interval", () => {
    const listener = vi.fn(); decoder.subscribe(listener); confirm(decoder, "ㄱ", 1);
    decoder.pushFrame({ capturedAt: 130, handPresent: false }); decoder.pushFrame({ capturedAt: 175, handPresent: false });
    expect(listener).toHaveBeenCalledWith(expect.objectContaining({ type: "HAND_RELEASED", previousSymbol: "ㄱ" }));
    expect(decoder.getSnapshot().state).toBe("NO_HAND");
  });

  it("does not release when only the hand position changes", () => {
    const listener = vi.fn(); decoder.subscribe(listener); confirm(decoder, "ㄱ", 1);
