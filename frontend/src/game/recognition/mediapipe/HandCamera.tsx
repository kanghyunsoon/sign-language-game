...
export function HandCamera({ sharedStream, rateConfig = DEFAULT_RECOGNITION_RATE_CONFIG, performanceMonitor, temporalDecoder, activePlayerSession, recognitionSession, handDetectionConfig = GAMEPLAY_HAND_DETECTION_CONFIG, autoStart = false, onLandmarkFrame, onHandNotDetected, targetSymbol, prediction, referenceTemplate, onPoseFeedback, connectionState, modelVersion, connectionError, awaitingHandRelease = false, showDebug = false, compact = false, visionAdapterFactory }: HandCameraProps) {
  ...
  const strongLandmarksRef = useRef<readonly import("../types/landmark").HandLandmark[] | undefined>(undefined);
  ...
  useEffect(()=>{monitorRef.current.start();const unsubscribe=monitorRef.current.subscribe(setPerformanceSnapshot);return()=>{unsubscribe();monitorRef.current.stop();};},[]);
  ...
  const releaseResources = useCallback((updateState: boolean) => {
    ...
    handBufferRef.current.clear();processingHandRef.current=false;renderHandsRef.current=[];lightLandmarksRef.current=undefined;strongLandmarksRef.current=undefined;
  ...
  const drawLatest=useCallback((frame:RecognitionVideoFrame)=>{const canvas=canvasRef.current,video=frame.video;if(!canvas)return;if(canvas.width!==video.videoWidth||canvas.height!==video.videoHeight){canvas.width=video.videoWidth;canvas.height=video.videoHeight;if(stageRef.current)stageRef.current.style.aspectRatio=`${video.videoWidth} / ${video.videoHeight}`;}const hands=renderHandsRef.current,template=feedbackHandlersRef.current.referenceTemplate;const overlayStates=hands.map((hand)=>{const result=template?compareTemplateToCurrentLandmarks(hand.landmarks,hand.handedness,template):null;return result&&result.availability==="AVAILABLE"&&template?{feedback:result.feedback,referenceLandmarks:alignTemplateToCurrentHand(template.landmarks,hand.landmarks,hand.handedness)}:{};});const context=canvas.getContext("2d");if(context)drawHandOverlay(context,hands,overlayStates);},[]);

  const processPendingHands=useCallback(async()=>{if(processingHandRef.current)return;processingHandRef.current=true;try{while(mountedRef.current){const frame=handBufferRef.current.takeLatest();if(!frame)break;const adapter=visionAdapterRef.current;if(!adapter)break;let hands:readonly TrackedHand[];try{hands=await adapter.detectHands({video:frame.video,timestamp:performance.now(),frameId:frame.frameId});monitorRef.current.mark("hand");setWorkerMode(adapter.getExecutionMode());}catch{setError({kind:"MEDIAPIPE_INIT_FAILED",message:"손 추적 처리에 실패했습니다. 영상과 게임은 계속됩니다."});break;}
    ...
    if(activePlayerSession){const candidates:HandCandidate[]=hands.map((hand,index)=>({detectionId:`hand-${frame.frameId}-${index}`,landmarks:hand.landmarks,handedness:hand.handedness,handednessScore:hand.handednessScore??0,wrist:hand.landmarks[0]!,detectedAt:frame.capturedAt}));if(recognitionSession){const ownership=recognitionSession.resolveHandCandidates(candidates,frame.capturedAt);setActiveHandSnapshot(ownership.ownership);const previewId=ownership.selected?.detectionId??ownership.ownership?.scores[0]?.handDetectionId,previewIndex=candidates.findIndex((candidate)=>candidate.detectionId===previewId);preview=previewIndex>=0?hands[previewIndex]:undefined;if(ownership.inputAllowed&&ownership.selected){const selectedIndex=candidates.findIndex((candidate)=>candidate.detectionId===ownership.selected!.detectionId);primary=hands[selectedIndex];activeSessionId=ownership.sessionId;activeHandId=ownership.activeHandId;}}else if(activeHandTrackerRef.current){const ownership=activeHandTrackerRef.current.update(candidates,activePlayerSession.getSnapshot(),frame.capturedAt);setActiveHandSnapshot(ownership);const previewId=ownership.selected?.detectionId??ownership.scores[0]?.handDetectionId,previewIndex=candidates.findIndex((candidate)=>candidate.detectionId===previewId);preview=previewIndex>=0?hands[previewIndex]:undefined;if(ownership.inputAllowed&&ownership.selected&&ownership.session){const selectedIndex=candidates.findIndex((candidate)=>candidate.detectionId===ownership.selected!.detectionId);primary=hands[selectedIndex];activeSessionId=ownership.session.sessionId;activeHandId=ownership.session.activeHandId;}}}
    else primary=hands[0];
    if(primary){if(activeSessionId&&lastActiveHandSessionIdRef.current!==activeSessionId){lightLandmarksRef.current=undefined;strongLandmarksRef.current=undefined;lastActiveHandSessionIdRef.current=activeSessionId;}const light=smoothHandLandmarks(lightLandmarksRef.current,primary.landmarks,.55);const strong=smoothHandLandmarks(strongLandmarksRef.current,primary.landmarks,.25);lightLandmarksRef.current=light;strongLandmarksRef.current=strong;renderHandsRef.current=[{...primary,landmarks:strong}];const output={frameId:frame.frameId,capturedAt:frame.capturedAt,handedness:primary.handedness,landmarks:light,rawLandmarks:primary.landmarks,activeHandId,activeHandSessionId:activeSessionId};if(recognitionSession)recognitionSession.submitLandmarkFrame(output);else handlersRef.current.onLandmarkFrame?.(output);const template=feedbackHandlersRef.current.referenceTemplate;if(template&&performance.now()-lastFeedbackReportAtRef.current>=FEEDBACK_REPORT_INTERVAL_MS){lastFeedbackReportAtRef.current=performance.now();feedbackHandlersRef.current.onPoseFeedback?.(compareTemplateToCurrentLandmarks(primary.landmarks,primary.handedness,template));}}else{renderHandsRef.current=preview?[preview]:[];lightLandmarksRef.current=undefined;strongLandmarksRef.current=undefined;if(recognitionSession)recognitionSession.notifyHandNotDetected(frame.capturedAt);else handlersRef.current.onHandNotDetected?.(frame.capturedAt);}if(hands.length!==lastHandCountRef.current){lastHandCountRef.current=hands.length;setHandCount(hands.length);}}}finally{processingHandRef.current=false;if(handBufferRef.current.hasPending())void processPendingHands();}},[activePlayerSession,recognitionSession]);
