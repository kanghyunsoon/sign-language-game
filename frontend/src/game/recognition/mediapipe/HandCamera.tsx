...
export function HandCamera({ sharedStream, rateConfig = DEFAULT_RECOGNITION_RATE_CONFIG, performanceMonitor, temporalDecoder, activePlayerSession, recognitionSession, handDetectionConfig = GAMEPLAY_HAND_DETECTION_CONFIG, autoStart = false, onLandmarkFrame, onHandNotDetected, targetSymbol, prediction, referenceTemplate, onPoseFeedback, connectionState, modelVersion, connectionError, awaitingHandRelease = false, showDebug = false, compact = false, visionAdapterFactory }: HandCameraProps) {
  ...
  const handRenderVersionRef = useRef(0);
  const lastDrawnHandVersionRef = useRef(-1);
  ...
  const releaseResources = useCallback((updateState: boolean) => {
    ...
    handBufferRef.current.clear();processingHandRef.current=false;renderHandsRef.current=[];handRenderVersionRef.current+=1;lastDrawnHandVersionRef.current=-1;lightLandmarksRef.current=undefined;strongLandmarksRef.current=undefined;
  ...
  const drawLatest=useCallback((frame:RecognitionVideoFrame)=>{const canvas=canvasRef.current,video=frame.video;if(!canvas)return;const resized=canvas.width!==video.videoWidth||canvas.height!==video.videoHeight;if(resized){canvas.width=video.videoWidth;canvas.height=video.videoHeight;if(stageRef.current)stageRef.current.style.aspectRatio=`${video.videoWidth} / ${video.videoHeight}`;}if(!resized&&lastDrawnHandVersionRef.current===handRenderVersionRef.current)return;const hands=renderHandsRef.current,template=feedbackHandlersRef.current.referenceTemplate;const overlayStates=hands.map((hand)=>{const result=template?compareTemplateToCurrentLandmarks(hand.landmarks,hand.handedness,template):null;return result&&result.availability==="AVAILABLE"&&template?{feedback:result.feedback,referenceLandmarks:alignTemplateToCurrentHand(template.landmarks,hand.landmarks,hand.handedness)}:{};});const context=canvas.getContext("2d");if(context){drawHandOverlay(context,hands,overlayStates);lastDrawnHandVersionRef.current=handRenderVersionRef.current;}},[]);
  ...
  const processPendingHands=useCallback(async()=>{if(processingHandRef.current)return;processingHandRef.current=true;try{while(mountedRef.current){const frame=handBufferRef.current.takeLatest();if(!frame)break;const adapter=visionAdapterRef.current;if(!adapter)break;let hands:readonly TrackedHand[];try{hands=await adapter.detectHands({video:frame.video,timestamp:performance.now(),frameId:frame.frameId});monitorRef.current.mark("hand");setWorkerMode(adapter.getExecutionMode());}catch{setError({kind:"MEDIAPIPE_INIT_FAILED",message:"손 추적 처리에 실패했습니다. 영상과 게임은 계속됩니다."});break;}
    ...
    if(primary){if(activeSessionId&&lastActiveHandSessionIdRef.current!==activeSessionId){lightLandmarksRef.current=undefined;strongLandmarksRef.current=undefined;lastActiveHandSessionIdRef.current=activeSessionId;}const light=smoothHandLandmarks(lightLandmarksRef.current,primary.landmarks,.55);const strong=smoothHandLandmarks(strongLandmarksRef.current,primary.landmarks,.25);lightLandmarksRef.current=light;strongLandmarksRef.current=strong;renderHandsRef.current=[{...primary,landmarks:strong}];handRenderVersionRef.current+=1;const output={frameId:frame.frameId,capturedAt:frame.capturedAt,handedness:primary.handedness,landmarks:light,rawLandmarks:primary.landmarks,activeHandId,activeHandSessionId:activeSessionId};if(recognitionSession)recognitionSession.submitLandmarkFrame(output);else handlersRef.current.onLandmarkFrame?.(output);const template=feedbackHandlersRef.current.referenceTemplate;if(template&&performance.now()-lastFeedbackReportAtRef.current>=FEEDBACK_REPORT_INTERVAL_MS){lastFeedbackReportAtRef.current=performance.now();feedbackHandlersRef.current.onPoseFeedback?.(compareTemplateToCurrentLandmarks(primary.landmarks,primary.handedness,template));}}else{renderHandsRef.current=preview?[preview]:[];handRenderVersionRef.current+=1;lightLandmarksRef.current=undefined;strongLandmarksRef.current=undefined;if(recognitionSession)recognitionSession.notifyHandNotDetected(frame.capturedAt);else handlersRef.current.onHandNotDetected?.(frame.capturedAt);}if(hands.length!==lastHandCountRef.current){lastHandCountRef.current=hands.length;setHandCount(hands.length);}}}finally{processingHandRef.current=false;if(handBufferRef.current.hasPending())void processPendingHands();}},[activePlayerSession,recognitionSession,showDebug]);
