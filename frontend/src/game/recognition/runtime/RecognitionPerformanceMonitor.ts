...
const EMPTY: RecognitionPerformanceSnapshot = { cameraFps:0,renderFps:0,handTrackingFps:0,poseTrackingFps:0,aiRequestFps:0,aiResponseFps:0,aiAverageLatencyMs:0,aiP95LatencyMs:0,droppedHandFrames:0,droppedPoseFrames:0,droppedInferenceFrames:0,staleResponsesIgnored:0,mainThreadLongTaskCount:0,latestUpdatedAt:0 };
...
export class RecognitionPerformanceMonitor {
  ...
  recordAiLatency(value:number):void{if(Number.isFinite(value)&&value>=0){this.latencies.push(value);if(this.latencies.length>240)this.latencies.splice(0,this.latencies.length-240);this.recalculate(this.now());}}
  ...
  dispose():void{this.stop();this.listeners.clear();Object.values(this.rates).forEach((value)=>value.splice(0));this.latencies=[];}
  private trim(values:number[],at:number){while(values.length&&values[0]!<at-1000)values.shift();}
  private recalculate(at:number){for(const values of Object.values(this.rates))this.trim(values,at);const sorted=[...this.latencies].sort((a,b)=>a-b);const average=sorted.length?sorted.reduce((a,b)=>a+b,0)/sorted.length:0;const p95=sorted.length?sorted[Math.min(sorted.length-1,Math.ceil(sorted.length*.95)-1)]!:0;this.snapshot={...this.snapshot,cameraFps:this.rates.camera.length,renderFps:this.rates.render.length,handTrackingFps:this.rates.hand.length,poseTrackingFps:this.rates.pose.length,aiRequestFps:this.rates.aiRequest.length,aiResponseFps:this.rates.aiResponse.length,aiAverageLatencyMs:average,aiP95LatencyMs:p95,latestUpdatedAt:Date.now()};if(at-this.lastPublishedAt>=250){this.lastPublishedAt=at;this.publish();}}
