...
export class BrowserRecognitionFrameScheduler implements RecognitionFrameScheduler {
  ...
  private tick=(at:number)=>{if(!this.running)return;if(!this.paused&&this.options.video.readyState>=2){const mediaTime=this.options.video.currentTime;if(mediaTime!==this.lastMediaTime){const now=this.now();this.lastMediaTime=mediaTime;this.frameId+=1;const frame={frameId:this.frameId,capturedAt:Date.now(),mediaTime,video:this.options.video};this.options.monitor?.mark("camera",now);this.emitDue("render",frame,now,this.config.renderFps);this.emitDue("hand",frame,now,this.config.handTrackingFps);this.emitDue("pose",frame,now,this.config.poseTrackingFps);}}this.handle=this.requestFrame(this.tick);void at;};
