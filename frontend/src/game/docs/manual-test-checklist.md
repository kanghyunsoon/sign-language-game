...
## 1:1 장시간 성능 회귀 — 2026-08-02

- [ ] 두 브라우저에서 각 보드에 글자를 15개 이상 쌓고 10분 이상 진행해도 입력과 낙하 애니메이션이 점차 느려지지 않는다.
- [ ] 정착 상태에서는 로컬·상대 보드의 전체 순회가 약 10FPS로 내려가고, 새 글자가 생기면 즉시 활성 프레임 주기로 복귀한다.
- [ ] 정착한 DOM 글자는 영구 `will-change` 합성 레이어를 보유하지 않는다.
- [ ] DOM 모드에서 누적 글자마다 보이지 않는 Pixi `LetterView`가 함께 증가하지 않는다.
- [ ] 128개를 넘긴 상대 제거 이력과 256개를 넘긴 P2P 명령 이력이 오래된 항목부터 정리된다.
- [ ] 재대결과 방 나가기 후 이전 게임의 rAF, DOM 글자, 제거 효과, 명령 이력이 남지 않는다.
- [ ] 성능 최적화 후에도 상대 낙하·제거·망치 전송과 제거 완료 이벤트가 누락되지 않는다.

## 프링글수 로비·결과 회귀 — 2026-08-02

- [ ] 방을 생성한 브라우저에서 카드에 입력한 방 제목, 현재 사용자 닉네임, 선택한 `자음`/`모음`/`기초 혼합` 범위가 표시된다.
- [ ] 생성한 방에서 로비로 돌아와 `재입장` 카드가 보일 때도 입력한 방 제목과 현재 방장 닉네임이 fallback 문구로 바뀌지 않는다.
- [ ] 방 생성 뒤 새로고침하거나 gateway가 재생성되어도 동일 브라우저에서는 입력한 제목·닉네임·출제 범위가 복원된다.
- [ ] 생성 시간이 없는 방 카드에 빈 생성 칸이나 `서버 정보 없음`이 표시되지 않는다.
- [ ] 서버 메타데이터가 없는 다른 브라우저에서도 초대 코드가 방 제목으로, `방장`이 닉네임으로 오인되어 표시되지 않는다.
- [ ] 두 명이 결과 화면에 머무는 동안 해당 `2/2 WAITING` 방이 게임방 찾기 목록에 나타나지 않는다.
- [ ] 결과 화면의 제목과 설명이 전체 모달 중앙에 있고 우측 최종 점수 카드 때문에 왼쪽으로 밀리지 않는다.
- [ ] 결과 화면에서 `다시 하기`와 `같은 방으로`는 같은 대기실로 돌아간다.
- [ ] 결과 화면에서 `게임방 목록`을 누르면 leave 후 `/game/battle`로 이동한다.
- [ ] 방장이 혼자 있는 방에서 브라우저 뒤로가기를 누르면 방이 제거되고 재입장 안내가 남지 않는다.
- [ ] 두 명인 방에서 방장이 나가면 서버가 남은 참가자에게 방장을 위임하고 대기실 표시가 갱신된다.
- [ ] 방을 나간 직후 새 방을 만들 때 `이미 참여 중인 방` 오류가 발생하지 않는다.
- [ ] 로컬에만 남은 방 세션의 서버 join이 `401/403/404/410`이면 세션을 폐기하고 같은 제출에서 새 방을 만든다.
- [ ] 1:1 게임판은 두 영역으로 분리되어 있으나 낮·밤 배경은 한 화면처럼 이어지고, 야간 중앙선이 이중으로 밝아지지 않는다.
- [ ] 양쪽 보드의 결승선이 같은 높이에 보이며 정착 블록이 해당 선에 닿았을 때 승패가 결정된다.
