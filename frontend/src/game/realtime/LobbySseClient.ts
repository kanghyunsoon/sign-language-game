...
export function parseLobbyRooms(value: unknown): readonly LobbyRoomSummary[] {
  ...
  return value.rooms.map((item) => {
    ...
    const hostName = optionalText(item.hostName);
    const symbolRange = item.symbolRange;
    if (symbolRange !== "CONSONANT" && symbolRange !== "VOWEL" && symbolRange !== "ALL") throw new Error("Invalid lobby symbolRange.");
    if (!title || !hostName) throw new Error("Invalid lobby room metadata.");
    return {
      id: integer(item.id),
