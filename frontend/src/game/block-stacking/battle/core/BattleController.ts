...
import type { AttackCreatedEvent, BattleConnectionOptions, BattleConnectionState, BoardSnapshotEvent, ComboUpdatedEvent, HammerAttackEvent, MatchFinishedEvent, ServerBattleMessage, SharedTargetClaimedEvent } from "../transport/battleTransportTypes";
...
export class BattleController {
  ...
  submitRecognizedSymbol(symbol: string, confidence = 1): boolean { if (this.machine.getState() !== "PLAYING" || !this.matchId) return false;
    if (this.options.sharedTargetMode) {
      ...
      if (symbol !== this.sharedTargetSymbol) { this.options.transport.send({ type: "RESET_COMBO_COMMAND", commandId: this.createCommandId(), matchId: this.matchId, reason: "WRONG_ANSWER", occurredAt: this.now() }); this.message = `목표 글자 ${this.sharedTargetSymbol}를 먼저 맞혀보세요.`; this.publish(); return false; }
