...
class FakeBoard implements BattleLocalBoard {
  ...
  acceptRemoval(id: string) { this.accepted.push(id); } rejectRemoval(id?: string) { this.rejected.push(id); } removeLetter(id: string) { this.accepted.push(id); return true; } getTargetSymbol() { return null; } takeTargetForOtter() { return null; } takeLetterForOtter() { return null; } resize() {} setPublisher(_publisher: LocalBoardPublisher) {} setGameOverHandler(handler: () => void) { this.gameOverHandler = handler; } dispose() { this.disposed = true; }
}
...
describe("BattleController", () => {
  ...
  it("restores a running match without replaying the countdown", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await connected(t, b); t.emit({ ...start, resume: true, playerStates: [{ playerId: "me", score: 240, combo: 3, maxCombo: 5, removedCount: 4, gameOver: false }] }); expect(c.snapshot()).toMatchObject({ state: "PLAYING", countdownMs: 0, score: 240, combo: 3, maxCombo: 5, removedCount: 4 }); expect(b.starts).toBe(1); c.dispose(); });
  it("restores the current shared target atomically with a running match", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await connected(t, b); t.emit({ ...start, resume: true, sharedTarget: { targetId: "target-7", symbol: "ㄱ", presentedAt: 2_000 } }); expect(c.snapshot()).toMatchObject({ state: "PLAYING", targetSymbol: "ㄱ" }); c.dispose(); });
  ...
  it("shows the shared idle selection before removing the exact local and remote letters", async () => {
    const t = new MockBattleTransport(); const b = new FakeBoard(); const remote = new RemoteBoardReplica(DEFAULT_BATTLE_SYNC_CONFIG);
    const removeRemote = vi.spyOn(remote, "removeLetter"); const selected = vi.fn(); const executed = vi.fn();
    const c = new BattleController({ playerId: "me", roomId: "r", transport: t, localBoard: b, remoteBoard: remote, attackEffect: attack(), sharedTargetMode: true, onIdleRemovalSelected: selected, onIdleRemovalExecuted: executed });
    await c.connect({ url: "ws://x", roomId: "r", playerId: "me" });
    const targets = [
      { playerId: "me", letterId: "local-idle", symbol: "ㅁ", normalizedX: .4, normalizedY: .8 },
      { playerId: "other", letterId: "remote-idle", symbol: "ㄹ", normalizedX: .6, normalizedY: .81 },
    ] as const;
    t.emit({ type: "IDLE_REMOVAL_SELECTED", sequence: 3, matchId: "m", removalId: "idle-1", targets, selectedAt: 1_000, executeAt: 2_150 });
    expect(selected).toHaveBeenCalledOnce(); expect(b.accepted).toHaveLength(0); expect(removeRemote).not.toHaveBeenCalled();
    t.emit({ type: "IDLE_REMOVAL_EXECUTED", sequence: 4, matchId: "m", removalId: "idle-1", targets, executedAt: 2_150 });
    expect(b.accepted).toContain("local-idle"); expect(removeRemote).toHaveBeenCalledWith("remote-idle"); expect(executed).toHaveBeenCalledOnce();
    c.dispose();
  });
  ...
  it("keeps the remaining player in the same session while it requests peer recovery", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); t.simulateConnectionState("DISCONNECTED"); expect(c.snapshot().state).toBe("PLAYING"); expect(b.stops).toBe(0); await vi.advanceTimersByTimeAsync(1000); expect(t.connections).toHaveLength(2); expect(t.sent.slice(-2)).toEqual([expect.objectContaining({ type: "PLAYER_RECONNECTED", matchId: "m" }), expect.objectContaining({ type: "REQUEST_MATCH_STATE", matchId: "m" })]); c.dispose(); });
  it("returns the remaining browser's latest interpolated opponent board after reconnection", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); t.emit({ type: "BOARD_SNAPSHOT", sequence: 12, matchId: "m", playerId: "other", sentAt: 500, bodies: [{ id: "other-1", symbol: "ㄱ", x: .5, y: .4, angle: 0, velocityX: 0, velocityY: 2, angularVelocity: 0, state: "FALLING" }] }); t.simulateConnectionState("DISCONNECTED"); await vi.advanceTimersByTimeAsync(1_000); expect(t.sent).toContainEqual(expect.objectContaining({ type: "PEER_BOARD_VIEW", observerPlayerId: "me", subjectPlayerId: "other", sentAt: expect.any(Number), bodies: [expect.objectContaining({ id: "other-1", x: .5, y: .4 })] })); c.dispose(); });
  it("keeps play active while a fresh opponent snapshot arrives", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); t.simulateConnectionState("DISCONNECTED"); await vi.advanceTimersByTimeAsync(1000); t.emit({ type: "BOARD_SNAPSHOT", sequence: 20, matchId: "m", playerId: "other", sentAt: Date.now(), bodies: [] }); expect(c.snapshot().state).toBe("PLAYING"); expect(b.starts).toBe(1); c.dispose(); });
  it("does not invent a winner from an ambiguous local transport drop", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); t.simulateConnectionState("DISCONNECTED"); await vi.advanceTimersByTimeAsync(10_000); expect(c.snapshot()).toMatchObject({ state: "PLAYING", result: null }); expect(b.stops).toBe(0); c.dispose(); });
  it("awards the remaining player after an identified opponent disconnect exceeds grace", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); t.emit({ type: "PLAYER_DISCONNECTED", sequence: 30, playerId: "other" }); await vi.advanceTimersByTimeAsync(10_000); expect(c.snapshot()).toMatchObject({ state: "FINISHED", result: expect.objectContaining({ winnerPlayerId: "me", loserPlayerId: "other", reason: "RECONNECT_TIMEOUT" }) }); expect(b.stops).toBe(1); c.dispose(); });
  it("keeps the match running when the opponent reconnects within grace", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); t.simulateConnectionState("DISCONNECTED"); await vi.advanceTimersByTimeAsync(1_000); t.emit({ type: "BOARD_SNAPSHOT", sequence: 20, matchId: "m", playerId: "other", sentAt: Date.now(), bodies: [] }); await vi.advanceTimersByTimeAsync(10_000); expect(c.snapshot()).toMatchObject({ state: "PLAYING", result: null }); c.dispose(); });
  it("cancels the leave timeout when the same match resumes", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); t.simulateConnectionState("DISCONNECTED"); t.emit({ ...start, matchId: "m", resume: true, playerStates: [{ playerId: "me", score: 0, combo: 0, maxCombo: 0, removedCount: 0, gameOver: false }] }); await vi.advanceTimersByTimeAsync(10_000); expect(c.snapshot()).toMatchObject({ state: "PLAYING", result: null }); c.dispose(); });
  ...
  it("keeps the final result when a late resume snapshot arrives", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); const result = { type: "MATCH_FINISHED" as const, sequence: 30, matchId: "m", winnerPlayerId: "me", loserPlayerId: "other", reason: "DANGER_LINE", finishedAt: Date.now() }; t.emit(result); t.emit({ ...start, sequence: 31, resume: true, startAt: Date.now(), serverTime: Date.now() }); expect(c.snapshot()).toMatchObject({ state: "FINISHED", result: expect.objectContaining({ winnerPlayerId: "me", loserPlayerId: "other" }) }); expect(b.starts).toBe(1); c.dispose(); });
  ...
  it("sends an authoritative forfeit while a match is playing", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); expect(c.forfeit()).toBe(true); expect(t.sent.at(-1)).toMatchObject({ type: "PLAYER_FORFEIT_COMMAND", matchId: "m" }); c.dispose(); });
...
async function connected(t: MockBattleTransport, b: FakeBoard, effect = attack()) { const c = new BattleController({ playerId: "me", roomId: "r", transport: t, localBoard: b, remoteBoard: new RemoteBoardReplica(DEFAULT_BATTLE_SYNC_CONFIG), attackEffect: effect, sharedTargetMode: true, createCommandId: () => "cmd" }); await c.connect({ url: "ws://x", roomId: "r", playerId: "me" }); return c; }
async function playing(t: MockBattleTransport, b: FakeBoard) { const c = await connected(t, b); t.emit({ ...start, startAt: 1000, serverTime: 1000 }); vi.runOnlyPendingTimers(); t.emit({ type: "SHARED_TARGET", sequence: 2, matchId: "m", targetId: "target-1", symbol: "ㄱ", presentedAt: Date.now() }); return c; }
