...
describe("BattleController", () => {
  ...
  it("awards the remaining player a win when reconnect grace expires", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); t.simulateConnectionState("DISCONNECTED"); await vi.advanceTimersByTimeAsync(10_000); expect(c.snapshot().state).toBe("FINISHED"); expect(c.snapshot().result).toMatchObject({ winnerPlayerId: "me", loserPlayerId: "other", reason: "RECONNECT_TIMEOUT" }); c.dispose(); });
