...
describe("BattleController", () => {
  ...
  it("returns the remaining browser's observed opponent board after reconnection", async () => { const t = new MockBattleTransport(); const b = new FakeBoard(); const c = await playing(t, b); t.emit({ type: "BOARD_SNAPSHOT", sequence: 12, matchId: "m", playerId: "other", sentAt: 500, bodies: [{ id: "other-1", symbol: "ㄱ", x: .5, y: .4, angle: 0, velocityX: 0, velocityY: 2, angularVelocity: 0, state: "FALLING" }] }); t.simulateConnectionState("DISCONNECTED"); await vi.advanceTimersByTimeAsync(1_000); expect(t.sent).toContainEqual(expect.objectContaining({ type: "PEER_BOARD_VIEW", observerPlayerId: "me", subjectPlayerId: "other", sentAt: 500 })); c.dispose(); });
