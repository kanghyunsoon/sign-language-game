...
export function BattleBoardPanel({ title, subtitle, toolbar, className, rendererConfig, towerHeightRatio = 0, dropBurst, idleRemovalTarget, ...canvasProps }: BattleBoardPanelProps) {
  ...
  return <section className={[styles.boardPanel, className].filter(Boolean).join(" ")} style={{ background: "transparent" }}><header><div>{subtitle ? <span>{subtitle}</span> : null}<h2>{title}</h2></div>{toolbar}</header><div className={styles.boardPlayArea} style={{ background: "transparent" }}><GameCanvas className={styles.boardCanvas} rendererConfig={stableRendererConfig} {...canvasProps} /><span className={styles.boardFinishLine} style={{ top: `${finishLineRatio * 100}%` }} aria-hidden="true" />{dropBurst ? <span key={dropBurst.id} className={styles.claimDropBurst} aria-hidden="true"><i/><i/></span> : null}{idleRemovalTarget ? <span key={idleRemovalTarget.letterId} className={styles.boardBlackHole} style={{ left: `${idleRemovalTarget.normalizedX * 100}%`, top: `${idleRemovalTarget.normalizedY * 100}%` }} aria-hidden="true"><i/><i/></span> : null}<TowerHeightGauge ratio={towerHeightRatio} className={styles.boardTowerHeightGauge} label="STACK" /></div></section>;
}
