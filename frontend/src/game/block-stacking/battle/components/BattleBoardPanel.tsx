...
export function BattleBoardPanel({ title, subtitle, toolbar, rendererConfig, ...canvasProps }: BattleBoardPanelProps) {
  ...
  return <section className={styles.boardPanel}><header><div><span>{subtitle}</span><h2>{title}</h2></div>{toolbar}</header><div className={styles.boardPlayArea}><div className={styles.battleSkyDecor} aria-hidden="true"><i className={styles.skyCloudOne}/><i className={styles.skyCloudTwo}/><i className={styles.skyCloudThree}/><i className={styles.skySun}/><i className={styles.skyHills}/><div className={styles.skyGlyphs}>ㄱ ㅜ ㄷ ㅎ</div></div><GameCanvas className={styles.boardCanvas} rendererConfig={stableRendererConfig} {...canvasProps} /></div></section>;
}
