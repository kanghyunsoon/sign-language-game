...
export interface MatchStartedEvent extends ServerEnvelope { readonly type: "START_MATCH" | "MATCH_STARTED" | "GAME_START"; readonly matchId: string; readonly roomId: string; readonly playerIds: readonly string[]; readonly startAt: number; readonly matchVersion?: string; readonly ruleVersion?: string; readonly serverTime?: number; /** True only when a reconnecting client must restore an already-running match. */ readonly resume?: boolean; /** Recovery snapshots are broadcast on the P2P channel, but only this player may apply them. */ readonly resumePlayerId?: string; readonly playerStates?: readonly { readonly playerId: string; readonly score: number; readonly combo: number; readonly maxCombo: number; readonly removedCount: number; readonly gameOver: boolean }[]; /** Included atomically on resume so a reconnect cannot miss the current paper glyph. */ readonly sharedTarget?: { readonly targetId: string; readonly symbol: string; readonly presentedAt: number }; }
...
export interface BoardSnapshotEvent extends ServerEnvelope { readonly type: "BOARD_SNAPSHOT"; readonly matchId: string; readonly playerId: string; readonly sentAt: number; readonly bodies: readonly BattleBodyTransform[]; readonly boardChecksum?: string; /** Present only for a reconnect hydration snapshot. Other players must ignore it. */ readonly restoreForPlayerId?: string; }
