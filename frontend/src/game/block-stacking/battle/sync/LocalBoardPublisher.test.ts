...
describe("LocalBoardPublisher", () => {
  ...
  it("publishes a final snapshot as soon as a letter settles", async () => { const t = new MockBattleTransport(); await t.connect({ url: "ws://x", roomId: "r", playerId: "p" }); const p = new LocalBoardPublisher(t, { ...DEFAULT_BATTLE_SYNC_CONFIG, snapshotPublishIntervalMs: 1000 }, "m", "p"); p.update(0, [state], 100, 100); p.update(100, [{ ...state, settled: true }], 100, 100); expect(t.sent.map((m) => m.type)).toEqual(["BOARD_SNAPSHOT", "BOARD_SNAPSHOT"]); });
});
