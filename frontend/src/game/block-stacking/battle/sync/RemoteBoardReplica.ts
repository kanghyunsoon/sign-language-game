...
export class RemoteBoardReplica {
  private readonly buffer: RemoteTransformBuffer; private readonly states = new Map<string, string>(); private readonly symbols = new Map<string, string>(); private lastSequence = -1; private senderClockOffsetMs: number | null = null;
  ...
  apply(message: RemoteSyncMessage, receivedAt: number): boolean {
    ...
    if (message.type === "BOARD_SNAPSHOT") {
      this.lastSequence = message.sequence; const ids = message.bodies.filter((body) => body.state !== "REMOVED").map((body) => body.id); this.buffer.restore(ids); this.states.clear(); this.symbols.clear();
      for (const body of message.bodies) if (body.state !== "REMOVED") { this.states.set(body.id, body.state); this.symbols.set(body.id, body.symbol); this.buffer.push(message.sequence, sampledAt, body); } return true;
    }
    if (message.type === "BODY_TRANSFORM_BATCH") {
      ...
      for (const body of message.bodies) { this.states.set(body.id, body.state); this.symbols.set(body.id, body.symbol); this.buffer.push(message.sequence, sampledAt, body); } return true;
    }
  ...
  clear(): void { this.buffer.clear(); this.states.clear(); this.symbols.clear(); this.lastSequence = -1; this.senderClockOffsetMs = null; }
