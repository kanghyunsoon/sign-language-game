...
export function BattleGamePage() {
  ...
  const waitingForFirstBlock = snapshot.state === "PLAYING" && !claimedSymbol && Boolean(snapshot.targetSymbol);
  const emptyBoardHint = (owner: "local" | "remote") => waitingForFirstBlock && (owner === "local" ? towerHeights.local : towerHeights.remote) < .01
    ? <><strong>{snapshot.targetSymbol}</strong><span>{owner === "local" ? "먼저 목표 지문자를 맞혀 첫 블록을 떨어뜨리세요" : "상대방이 목표 지문자를 맞히면 블록이 쌓입니다"}</span></>
    : null;
