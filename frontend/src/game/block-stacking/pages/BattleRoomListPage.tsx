...
export function BattleRoomListPage({ mode = "BLOCK" }: { readonly mode?: "BLOCK" | "TURN" }) {
  ...
  return <main className={[styles.page, styles.lobbyPage].join(" ")}>
    ...
    <div className={styles.lobbyGrid}>
      ...
      <aside className={styles.lobbySide}><section className={styles.createPanel}><header><span><Plus aria-hidden="true" size={17} /> 게임 시작</span></header><button type="button" className={styles.createButton} onClick={() => setModalOpen(true)}>방 만들기</button><p>또는</p><div className={styles.codeJoin}><strong>초대 코드로 입장</strong><div><input value={roomCode} onChange={(event) => setRoomCode(event.target.value)} placeholder="ABCD12" aria-label="초대 코드" /><button type="button" onClick={joinByCode} disabled={!roomCode.trim() || joiningRoomId !== null}>{"\uCF54\uB4DC \uC785\uC7A5"}</button></div></div><button type="button" className={styles.linkJoin} onClick={() => setModalOpen(true)}>초대 링크로 입장</button></section><section className={styles.quickTip}><strong>빠른 입장</strong><p>현재 선택한 게임에서 바로 입장 가능한 공개방을 자동으로 찾아드려요.</p><button type="button" onClick={() => { const room = rooms.find((item) => item.canJoin); if (room) void joinRoom(room.roomId); }}>입장 가능한 방 찾기</button></section><section className={styles.modeTip}><strong>{mode === "TURN" ? "수달 턴 대전" : "지문자 테트리스"}</strong><p>화면에 나타나는 지문자를 표현해 블록을 제거하는 1대1 게임이에요.</p></section></aside>
    </div>
