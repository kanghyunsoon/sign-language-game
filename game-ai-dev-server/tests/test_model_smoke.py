...
from app.model_adapter import EXPANDED_LABELS, LABELS, TFLiteModelAdapter, TreeModelAdapter, create_model_runner
...
class TFLiteModelSmokeTests(unittest.TestCase):
    ...
    def test_baseline_is_default_and_exposes_jamo_labels(self) -> None:
        ...
        self.assertEqual(adapter.contract.labels, LABELS)
        self.assertEqual(adapter.predict(np.zeros((1, 10, 55), dtype=np.float32)).shape, (31,))
