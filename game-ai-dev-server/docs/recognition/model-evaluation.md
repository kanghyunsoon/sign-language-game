...
## 지문자·지숫자 정확도 — 최종 요약 (T-138~T-146)

- **지문자 자음:** 확실히 잡힘 — 취약해 보이던 `ㄱ·ㄹ·ㅊ·ㅋ·ㅌ`은 작은 test 착시였고, 대형 leakage-safe test에서 **92~100%**(T-142).
- **지문자 모음:** 대부분 ≥90%, 유사 모음(`ㅔ/ㅖ/ㅕ`, `ㅜ/ㅟ/ㅝ`, `ㅣ`, `ㅐ`)은 0.64~0.96로 **런 변동이 크고 레시피로 안정화 불가** — 데이터 다양성 병목(T-143/144/146).
- **지숫자:** 하이브리드(자모=이미지, 숫자=landmark)로 숫자 도메인 **97%대**, 숫자 head는 **KNN(k=7)** 채택 시 min-recall 0.75→0.833. 유일 취약 `NUM_9`(digit 9)는 방법 무관 ~0.833 상한 → 데이터 병목(T-141/145).
- **결론:** 손실·margin·표현·분류기 등 **알고리즘 레버는 소진**. 추가 정확도(유사 모음·`NUM_9` ≥90%)는 **표적 데이터 확보**(신규 signer·각도)로만 가능하며, 이는 로그인·승인이 필요해 사용자 주도로 진행해야 한다.
