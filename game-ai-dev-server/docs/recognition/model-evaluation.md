...
### T-150 실행 — 자모 전용 재편 + feature_v3 방향 피처 (배포 아키텍처, 채택)

- **배경·방향 수정:** T-142~T-148의 이미지 EfficientNet 트랙은 배포 서버(랜드마크 시퀀스 입력)와 아키텍처가 달라 드롭인 불가였다(T-149 정정). 또한 숫자 인식이 **별도 모델로 분리**되기로 결정돼, 이 회차는 **배포 계약(랜드마크 시퀀스) 위에서 자모 31-class 전용**으로 재편한다.
- **데이터:** 자모 촬영 영상 31개(`output_video/<자모>/<자모>_1.avi`, JunYong-Choi/Sign_Language_Translation 기반 캡처)에서 MediaPipe로 랜드마크(x,y,z) 재추출 → **feature_v3(78값 = 3D bone direction 60 + angle 15 + palm normal 3)** 시퀀스 3359개(train 2404 / test 955, 영상 앞 70% train·뒤 30% test). 기존 `seq_*.npy`는 feature_v2(55값)로 이미 가공돼 방향 정보를 복원할 수 없어 영상에서 다시 뽑았다. 산출물 `extract_v3_video.py`, `data/v3_video_seq.npz`.
- **기법·비교 대상:** LSTM(96)+Dense(48) / balanced 증강 / Adam 1e-3 / early stop. 비교군 (a) 배포 Tree 재현(feature_v2, 자모 82.6%), (b) 자모전용 Tree(87.3%), (c) baseline h5 가중치 전이 LSTM(feature_v2, 89.8%), (d) **본 회차 feature_v3 LSTM**.
- **실측(locked test 955):**

  | 모델 | 피처 | 자모 accuracy | min-recall |
  | --- | --- | --- | --- |
  | 배포 Tree 재현 | v2(55) | 82.6% | 0.12 |
  | 자모전용 Tree | v2(55) | 87.3% | 0.06 |
  | baseline 전이 LSTM | v2(55) | 89.8% | 0.42 |
  | **feature_v3 LSTM (채택)** | **v3(78)** | **93.3%** | **0.52** |

- **취약 자모 before→after (v2 전이 → v3, 핵심 정량):** `ㅅ` 0.42→**1.00**, `ㅁ` 0.70→**1.00**, `ㅠ` 0.59→**1.00**, `ㅔ` 0.53→**0.97**, `ㅓ` 0.75→**0.97**. 31개 중 **22개가 0.93~1.00**. 잔존 <0.90(9개): `ㅕ` 0.52, `ㄱ` 0.66, `ㄹ`·`ㅗ` 0.77, `ㅐ` 0.81, `ㅈ`·`ㅑ` 0.83, `ㅚ` 0.84, `ㅜ` 0.87. 최다 혼동: `ㅡ→ㅔ`, `ㅓ→ㄴ`, `ㅚ→ㅐ`.
- **판정·계약 변경:** **feature_v3 채택.** 2D 실루엣이 같아 v2에서 원리적으로 못 갈리던 자모(ㅅ/ㅠ, ㅔ/ㅕ 계열)가 방향·깊이 정보로 실제 분리됨을 실증했고, 손등 방향·상하 반전 판별 근거(palm normal)도 모델 입력에 포함됐다. 서버는 `feature_adapter`를 v3로 스위치(55→78), 프론트는 이미 x·y·z를 전송하므로 **프론트 변경 없음**. 운영 번들 `models/jamo-31-v3/`.
- **한계:** 자모당 영상 1개뿐이라 **signer-independent가 아니고**(같은 영상 앞/뒤 분할) 낙관적일 수 있다. 잔존 9개 자모와 실사용 강건성(가림·겹침)은 **신규 signer 촬영**으로 재검증·보강해야 한다. MediaPipe가 랜드마크를 잘못 주는 가림 상황은 모델 밖 문제로, 신뢰도·시간축 보정이 별도로 필요하다.

### T-151 실행 — 듀얼 헤드 앙상블 + 하드네거티브 (운영 채택, 98.7%)

- **동기:** T-150 v3 단독(93.3%)은 `ㅅ·ㅁ·ㅠ·ㅔ`를 살렸지만 `ㅕ·ㅗ·ㄹ`이 v2보다 낮았다. 원인은 **데이터량** — v3는 영상 1개(2404 시퀀스), v2 계열은 세션 seq 33738개로 학습됐다. 사용자 경험은 평균이 아니라 **최악 클래스**가 좌우하므로, 전 자모 고름을 목표로 잡았다.
- **1) v3 증량(무효에 가까움):** 프레임 stride 2→1로 시퀀스 3359→6693. accuracy 93.3%→93.0%, `<0.90` 9→7개. 인접 프레임 복제라 새 정보가 아님이 확인됨(`ㄱ` 0.86→0.93, `ㅎ` 0.83→0.91은 개선).
- **2) 사후 가중(무효):** v2 모델은 55차원이라 v3 test 프레임에 직접 못 돌려, class-prior 보정에 그침 → 93.0%→93.1%. **같은 프레임에서 두 피처를 모두 뽑아야** 진짜 앙상블이 됨을 확인.
- **3) 듀얼 헤드 앙상블(핵심):** 같은 영상 프레임에서 v2(55)·v3(78)를 **동시 추출**(`extract_both.py`)하고, (A) v2 헤드는 **세션 seq 33738 + 영상**으로, (B) v3 헤드는 영상으로 학습한 뒤 **확률 평균(w=0.5)**. 결과 **98.1%**, `<0.90` 2개(`ㅜ` 0.708, `ㅗ` 0.867). v2가 강한 자모와 v3가 강한 자모가 정확히 상호보완.
- **4) 하드네거티브 margin 1차:** `ㅜ` 오답 19/65가 전부 `ㅏ`로 몰린 것을 확인하고 `ㅜ-ㅏ` 등에 margin 0.35 손실 추가 → **`ㅜ` 0.708→1.00**, 전체 98.5%. 다만 margin을 안 건 `ㅓ`가 0.967→0.738로 밀리는 **풍선효과** 발생.
- **5) 하드네거티브 2차(채택):** 혼동행렬에서 쌍을 **자동 산출**(`ㅣ-ㅡ, ㅗ-ㅑ, ㅗ-ㅖ, ㅗ-ㅣ, ㅜ-ㅏ`)하고 margin을 0.35→0.25로 완화 → **전체 98.68%, 최저 recall 0.831**, `ㅓ` 회복. `<0.90`은 `ㅗ` 0.867 / `ㅜ` 0.831 둘뿐.
- **경과 요약:**

  | 단계 | 전체 accuracy | 최저 recall | <0.90 |
  | --- | --- | --- | --- |
  | v3 단독(T-150) | 93.3% | 0.52 | 9 |
  | 듀얼 앙상블 | 98.1% | 0.708 | 2 |
  | HN 1차 | 98.5% | 0.738 | 2 |
  | **HN 2차(채택)** | **98.7%** | **0.831** | **2** |

- **배포 형태:** `models/jamo-31-ensemble-v1/` — `jamo31-v2big.tflite`(55) + `jamo31-v3.tflite`(78). **LSTM unroll 변환으로 Flex(Select TF ops) 의존 제거**(Keras 원본과 maxdiff 1.5e-07). 서버는 `feature_adapter.landmarks_to_features`로 프레임마다 두 피처를 만들고 `EnsembleModelAdapter.predict_pair`로 평균한다. 프론트는 이미 x·y·z를 보내므로 **프론트 변경 없음**.
- **한계:** 자모당 영상 1개 기반이라 **signer-independent가 아니다**(같은 영상 앞/뒤 분할). 98.7%는 이 조건에서의 값이며, 실사용 강건성(가림·겹침, 새 signer)은 신규 촬영으로 재검증해야 한다. 남은 `ㅗ·ㅜ`도 표본이 60~65개라 0.83과 0.90의 차이는 오답 몇 건 수준이다.

### T-152 실행 — 회전 증강으로 실사용 `ㅡ` 실패 해소 (채택, jamo-31-ensemble-v2)

- **문제 제기(실사용):** T-151 배포 후 실사용에서 **`ㅡ`가 거의 인식되지 않는다**는 보고. 그런데 locked test에서 `ㅡ`는 recall 1.00, 앙상블 confidence 0.995, 프론트 임계값(0.5) 미달 0%였다 → **모델·임계값 문제가 아니라 test에 없는 조건(손목 각도)에서 실패**하는 것으로 진단.
- **원인:** `ㅡ`는 손을 눕혀 만드는 수평 형태이고, `ㅡ/ㅣ/ㅜ`는 서로 **회전 관계**다. 학습 데이터가 촬영 영상 1개뿐이라 그 영상의 특정 손목 각도만 학습했고, feature_v2는 화면 기준 방향을 그대로 쓰므로 사용자가 손을 기울이면 학습 분포를 벗어난다. test는 같은 영상 앞/뒤 분할이라 이 취약점이 드러나지 않았다.
- **기법:** 학습 샘플의 60%에 **손목 회전 증강**을 적용 — v2는 20개 2D bone vector를 in-plane 회전(±22°), v3는 3D direction과 palm normal을 z축 회전(±22°) + y축 회전(±15°)으로 변환. 하드네거티브 쌍(`ㅣ-ㅡ` 포함)은 유지. 산출물 `rot_train.py`, `code-v3/outputs/rot-final/`.
- **실측(locked test 1900):** accuracy 98.68%→**98.42%**, `<0.90` 2개→**1개**(`ㅓ` 0.705), min-recall 0.831→0.705.
- **회전 강건성(핵심 검증 — test를 실제로 회전시켜 재측정):**
  ...
  손목을 ±20° 기울여도 `ㅡ`가 100% 유지되고 전체 정확도도 흔들리지 않는다. 실사용 실패의 직접 원인에 대한 처방이 확인됨.
- **실패한 후속 시도(기록):** 잔존 `ㅓ` 0.705를 잡으려 `ㅓ-ㅡ`·`ㅓ-ㅕ` margin 추가 + 회전폭 완화(±16°)로 재학습(`rot2`) → **역효과**. `ㅓ`는 0.705→0.754로 소폭 올랐으나 **`ㅡ`가 1.00→0.707로 붕괴**, 전체 96.68%, `<0.90` 4개. `ㅓ-ㅡ` margin이 양방향으로 작용해 `ㅡ`를 깎았다. **폐기.**
- **판정:** **rot 1차본 채택 → `jamo-31-ensemble-v2`.** 실사용에서 확인된 실제 문제(`ㅡ` 각도 민감성)를 해결하는 대가로 test 수치 0.26%p와 `ㅓ` recall을 내주는 교환이며, 사용자 경험 기준에서 옳은 선택으로 판단. `ㅓ`는 **실사용에서 실제 문제로 확인되면** margin이 아니라 `ㅓ` 표본 확충으로 대응한다(margin 접근은 T-151/T-152에서 반복적으로 풍선효과를 냈다).
- **배포:** TFLite는 LSTM unroll로 재변환해 Flex 의존 없음(maxdiff 1.0e-07 / 9.3e-09). 인프라가 `/opt/sudal/ai/models/jamo-31-ensemble-v1/`에 tflite를 배치하는 구조이므로 **같은 경로의 파일을 교체**하면 된다(디렉터리명 유지, manifest의 modelVersion만 v2).

## 지문자·지숫자 정확도 — 최종 요약 (T-138~T-152)
...
- **지표 (모두 orientation-invariant):**
  ...
  - `thumb_clearance` = 엄지 끝(4)과 **네 손가락 끝** 중 최근접 거리(palm 단위). straightness만으로는 **곧게 펴서 접힌 손가락에 붙인 주먹**을 놓치기 때문에 필요하다.
  - `thumb_abduction` = (2→4)와 (5→8) 사이 각. `ㅂ`을 **곧은 엄지를 손바닥 옆에 붙여** 표현하는 경우를 살려주기 위해 `ㅂ` 거부의 **추가 필수 조건**으로 쓴다.
  - `finger_curl` = (mcp→pip)와 (dip→tip) 사이 각. 45°↓ 펴짐 / 95°↑ 접힘, **사이는 판정 보류**.
- **거부 규칙 (거부만 하고 승격은 절대 안 함):**
  - `ㅎ` — 네 손가락이 **명확히** 접힌 상태에서 `straightness < 0.85` **또는** `clearance < 0.40`
  - `ㅂ` — 네 손가락이 **명확히** 펴진 상태에서 `straightness > 0.93` **그리고** `abduction > 35°`
- **한계 — 임계값은 캘리브레이션 안 됨.** 저장소에 랜드마크 데이터셋이 없다(`ai/data`는 빈 수집 템플릿). 기본값은 **손 비율에서 유도한 값**이며 측정된 분포가 아니다. 그래서 모든 거부를 **논리곱 + 넓은 데드밴드**로 짜서 "게으른 손모양이 명백할 때만" 발동하게 했고, 임계값 4개 전부 env var다(`HANDPRACTICE_AI_HANDSHAPE_GATE=0`으로 전체 비활성). 실측 대체 절차는 `scripts/calibrate_handshape_gate.py` — 게이트를 끈 채 진단 JSONL을 캡처해 4가지 손모양의 백분위를 뽑고, `ㅎ` 행이 **이봉분포**인지 확인한 뒤 값을 바꾼다.
- **테스트:** `tests/test_handshape_gate.py` 15건 + `tests/test_recognition_session.py`에 배선 5건. 합성 표준 포즈 4종(정상 `ㅎ`/`ㅂ`, 맨주먹, 활짝 편 손) + 함정 2종(곧지만 붙인 엄지 주먹, 곧은 엄지 `ㅂ`). 마진은 여유롭다 — straightness 0.973 vs 0.738, clearance 0.551 vs 0.103. **43건 통과**(`test_model_smoke.py`는 이 환경에 tensorflow가 없어 미실행, 변경과 무관).
- **판정:** **모델·margin 게이트·확정 임계값·tflite 가중치 전부 무변경.** `PREDICTION`에 선택 필드 `handshapeHint`(코칭 문구)가 추가됐을 뿐이므로 무시하는 클라이언트는 기존 동작 그대로다 — 억제된 confidence만으로 이미 확정이 막힌다. **서버만 재배포**하면 된다.

## T-158 — 손모양 게이트 비활성화: 판별 지표가 클래스를 가르지 못한다

T-157에서 추가한 기하 검증 게이트를 기본 비활성으로 되돌렸다. 임계값이 틀린 게
아니라 `ㅂ` 판별에 쓴 지표가 주장한 것을 측정하지 않는다.

배포된 앱의 화면 녹화에서 웹캠 패널을 잘라내 MediaPipe를 다시 돌려 랜드마크를
복원했다. **정상** `ㅂ`(네 손가락 펴고 엄지를 손바닥 위로 접은 자세) 5프레임:

| 시각 | 손가락 curl (4개) | `four_fingers_extended` | straightness | abduction |
|---|---|---|---|---|
...
**1. `thumb_straightness`는 클래스를 가르지 못한다.** 코드 주석은 접은 엄지가
0.6~0.75로 떨어진다고 예상했는데 실측은 0.93~1.00이다. `ㅂ`의 엄지 접기는 안장
관절의 모음(adduction)이라 마디는 곧게 유지되고 현/사슬 길이 비율이 거의 움직이지
않는다. 같은 파일에 "엄지는 curl이 아니라 안장관절의 회전"이라고 써두고 판별식을
마디 굽힘으로 만든 것이 원인이다.

**2. `ㅂ` 거부 규칙이 정상 `ㅂ`에 발동한다.** 조건은 `straightness > 0.93` AND
`abduction > 35°`다. 실측 straightness는 항상 0.93을 넘고 abduction은 9°~36°에
흩어진다. 1.3s 프레임이 실제로 거부된다. 게으른 자세를 막으려면 정상 자세도 막게
되므로, 켜두면 false accept를 false reject로 바꾸는 거래가 된다.

**3. `all()` 전제조건이 동작 중간에 깨진다.** 0.2s 프레임은
`four_fingers_extended`가 False라 엄지 검사에 도달조차 못 한다.

T-157의 테스트가 통과한 이유는 합성 포즈가 임계값을 만족하도록 만들어졌기
때문이다. 손가락 각도가 정확히 0°/156°, 접은 엄지 straightness 0.67 — 실제로
관측되지 않는 값이다. 배선만 검증했고 판별력은 검증하지 못했다.

`ㅎ`(주먹) 쪽 실측은 확보하지 못했다. 정답 팝업이 뜨면 배경이 블러 처리돼
MediaPipe가 손을 검출하지 못한다.

측정 코드와 거부 배선은 남겼다. abduction 단독으로는 아직 가능성이 있지만,
**게으른 자세를 포함한 캡처**가 필요하다. 양쪽 클래스를 캘리브레이션하지 않고
재활성화하면 안 된다. 절차는 `scripts/calibrate_handshape_gate.py`.

한 가지 더 — `docker-compose.yml`이 `HANDPRACTICE_AI_HANDSHAPE_GATE`를 컨테이너에
넘기지 않는다. 배포 환경에서는 코드 기본값만 유효하고 env var로 껐다 켤 수 없다.
T-157 MR에 적어둔 "문제 생기면 =0으로 즉시 비활성"은 사실이 아니었다.

## T-159 — `ㅂ`/`ㅎ` 클래스별 확정 임계값 상향

T-158에서 기하 게이트를 접은 뒤, 배포된 앱 자신의 인식률 출력을 자세별로 읽었다.
모델은 이미 두 자세를 구분하고 있었고 confidence로 표현하고 있었다. 게이트도
재학습도 필요 없었다.

### 측정 — 앱 화면의 인식률

`ㅂ` (한 명, 1세션, 화면 녹화 28프레임을 자세별로 육안 라벨링):

| 자세 | 인식률 |
|---|---|
| 게으른 `ㅂ` (손가락 벌림 + 엄지 밖으로) | 64 67 68×5 69 70 70 71 71 72×3 78×3 **88** |
| 정상 `ㅂ` (손가락 모음 + 엄지 접음) | **97** 98 99×5 100 |

**88%와 97% 사이가 비어 있다.** 임계값 0.93이 그 사이에 앉는다.

`ㅎ` — 첫 관찰은 정상 0.85 이상, 게으른 최대 0.7이었다. 배포 후 사용자가 다시
측정한 결과 정상 `ㅎ`도 97% 이상이 나와, `ㅂ`과 같은 근거로 동일한 값을 쓴다.
게으른 `ㅎ`의 상한 0.7과는 25%p 이상 떨어져 있다.

### 왜 이게 되고 앞선 시도는 안 됐는가

- **전역 임계값 상향**은 안 된다. 문서에 이미 측정돼 있다 — 0.6에서 `ㅅ`이 프레임의
  16%를 잃는다. 그래서 **클래스별로** 올린다. 31개 중 2개만 건드리고 29개는 0.5
  그대로다. 클래스별 임계값이 계약에 존재하는 이유가 이것이다.
- **margin 게이트**로는 안 잡힌다. 실측한 게으른 `ㅂ` 프레임의 top1−top2 격차가
  **0.563**이었다. 문서의 가장 엄격한 설정이 0.40이고, 0.57까지 올리면 정상 프레임의
  gap 5분위(0.293)를 크게 넘어 정상 입력을 대량으로 막는다. margin은 *모호한* 오류를
  잡는 장치이고, 이 실패는 *확신 있는* 오류다.
- 두 장치는 상보적이다. margin은 혼동으로 인한 모호한 프레임을, 클래스 임계값은
  자세가 게을러서 생기는 확신 있는 오수락을 막는다.
...
0.97이 아니라 0.95인 이유는 여유다. 관측된 정상 자세의 **최저값이 정확히 97%**이므로
임계값을 그 위에 놓으면 가장 약한 정상 프레임이 경계에 걸린다. 게으른 자세의 상한이
`ㅂ` 88%, `ㅎ` 70%이므로 0.95로도 완전히 막히면서 2%p 여유가 남는다.

`CAPABILITIES`의 `confidenceThresholds`로 프론트엔드에 전달되고, 확정 권한은
`FRONTEND_TEMPORAL_DECODER`에 있다.

### 검증되지 않은 위험

1. **표본이 한 사람, 한 세션, 한 조명·거리다.** 다른 사용자의 정상 `ㅂ` 하한이 0.93
   아래면 그 사용자는 정답 처리가 되지 않는다. 배포 후 여러 명의 정상 `ㅂ`/`ㅎ`
   최저값만 확인하면 되고, 어긋나면 숫자만 조정한다.
2. **프론트엔드가 클래스별 맵을 실제로 적용하는지 서버 쪽에서 확인할 수 없다.**
   `confidenceThresholds`를 무시하고 상수를 쓰고 있으면 이 변경은 무효다. 배포 후
   게으른 자세가 여전히 통과하는지로 판별된다.
3. 정상 자세의 확정이 약간 느려질 수 있다. 임계값 아래 프레임이 버려지므로 디코더가
   프레임을 더 모아야 한다.

### T-159 결과 — 롤백. 측정 라벨이 틀렸다

`ㅂ`/`ㅎ`을 0.95로 올린 뒤에도 게으른 자세가 정답 처리됐다. 배선은 전부 정상이었다.
`CAPABILITIES`가 클라이언트에 `ㅂ: 0.95`를 실제로 전달하고 있음을 브라우저에서
확인했고, 프론트엔드 체인도 정상이다.

```
서버 CAPABILITIES {ㅂ:0.95}
  -> PracticeWebSocketSignRecognizer: decoder.updateConfig({minimumConfidenceBySymbol})
  -> ContinuousSignDecoder.validSample: confidence < floor 이면 폐기
  -> SIGN_CONFIRMED 미발행 -> 정답 처리 안 됨
```

그런데도 통과했다. 사용자 확인 결과 **손을 쫙 벌린 자세도 99%가 나온다.** 즉 T-159의
"게으른 88 / 정상 97" 분리는 존재하지 않는다.

원인은 라벨링이다. 화면 녹화 프레임을 내가 눈으로 보고 자세를 분류했고, 97~100%
프레임을 "손가락 모은 정상 `ㅂ`"으로 판정했다. 실제로 자세를 만든 사람의 확인과
어긋난다. 자세 라벨은 관찰자가 아니라 수행자가 정해야 한다.

오늘 같은 실패를 세 번 반복했다 — T-157의 합성 포즈(내 임계값을 만족하도록 내가 만든
포즈), T-158 분석에서 A 영상 전체를 게으른 자세로 일괄 라벨링, 그리고 T-159의 고저
분류. 전부 내가 만든 라벨을 근거로 삼았다. 앞으로 자세 라벨이 필요한 측정은 수행자가
구간을 명시한 데이터로만 한다.

confidence 기반 접근은 종료한다. 임계값을 0.5로 되돌린다. 서버가 하한을 직접 집행하는
변경(T-160)도 폐기한다 — 분리가 없으므로 집행할 하한 자체가 없다.
