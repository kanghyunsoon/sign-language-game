...
def capabilities_message(
    ...
    if readiness is not None:
        ...
        message["competitiveSymbols"] = [str(item["symbol"]) for item in classes if isinstance(item, dict) and item.get("competitiveEligible") is True]
        message["confidenceThresholds"] = {str(item["symbol"]): float(item["threshold"]) for item in classes if isinstance(item, dict) and isinstance(item.get("threshold"), (int, float))}
    return message
