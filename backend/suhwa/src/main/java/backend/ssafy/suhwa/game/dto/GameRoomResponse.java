...
public record GameRoomResponse(
    ...
    public static GameRoomResponse from(GameRoom room, String hostName) {
        return from(room, hostName, null);
    }

    /** 생성/입장/재입장 응답처럼 실시간 연결용 티켓을 함께 내려줘야 할 때 사용한다(FR-001/002/016). */
    public static GameRoomResponse from(GameRoom room, String hostName, String realtimeTicket) {
