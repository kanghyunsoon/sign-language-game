...
- **FR-004**: 익명/미인증 컨텍스트의 타입 처리 실패가 500이 아닌 정의된 인증 실패 응답으로 이어져야 한다. **(현행 검증 결과 조치 불요)** — `SecurityConfig`가 `.anyRequest().authenticated()`로 보호 경로를 리졸버 도달 전에 401 차단하고, `permitAll` + `@LoginUser` 조합이 없어 익명 principal이 리졸버까지 도달하는 경로가 존재하지 않음. 별도 방어 코드는 도입하지 않음.
